% Extracts the color-sets along different lines running across the flag templates %
% The description of color-sets can be found in Paper-, from %
...
void DuplicateFeatureSet(char[223][31]);
int minimum(int a, int b, int c);
int levenshtein_distance(char s[31], char t[31]);
int getRecognizedFlag(char featureSet[31]);
void readFeatures();
FILE* filesave;			// declared as a global variable
char allFlagFeatures[223][31];
int main()
{	
	//Feature Set
	char* ColorSet;
	...
	filesave = fopen("--path_of_file---\\ColorDetect.txt","w");
	char WholeFeatureSet[223][31];
	int count = 0;
	/*Filename variables */
	char filename[261] = {}; 
	char fullpath[100];
	...
	DIR *dir;
	struct dirent *ent;
	char dirpath[] = "--path_of_data--\\flags_large";
    dir = opendir (dirpath);
	readFeatures();
	if (dir != NULL) 
	{
		...
		while ((ent = readdir (dir)) != NULL) 
		{
			...
			{
				strcpy(fullpath,"");
				strcat(fullpath,"--path_of_data--\\flags_large\\");
				IpImage_flag = cvLoadImage(strcat(fullpath,ent->d_name),CV_LOAD_IMAGE_COLOR);
				/*strcat(fullpath,"--path_of_data--\\flags_style1_large\\Czech_Republic.png");
...
char* ColorSetAlongLine(CvArr* IpImage_flag_HSV, CvPoint pt1, CvPoint pt2)
{
	...
	/*	printf("%d,", iterator.ptr[0]); 
		...
		if(s<13) % for saturation<13 color is not perceptible 
		{ // grey scale image 
			if(v<120){
		...
		else if(s>13) % for saturation>13 color is perceptible 
		{
			// color image
			if(v<26){
				//add black (0)- if not already present
...
void DuplicateFeatureSet(char features[223][31])
{
	int ret=-1;
	int check[223] = {0};
	filesave = fopen("--path_of_file\\ColorSetData.txt","a");
	for(int i=0;i<223;i++)
	{
		if(check[i]==1)
			continue;
...
void readFeatures()
{
	...
	int countFeatures = 0;
	char colorset[31];
	FILE *fr = fopen("--path_of_file--\\ColorSetData.txt","rb");
	int noOfFeatures = 0;
...
int getRecognizedFlag(char testFeatureSet[31])
{
	...
	for(int i = 0; i < 223;i++)
	{
		int distance = levenshtein_distance(allFlagFeatures[i],testFeatureSet);
